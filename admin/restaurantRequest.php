...
if(@$_GET['removeRestaurantRequest']=="ok")
{
       ...
       $ch = curl_init( $baseurl.'/deleteRestaurantRequest' );
       ...
       if($http_code !== 200)
       {
         ...
         echo "<script>window.location='restaurantRequest.php?status=error';</script>";
       
       }
       else
       {
         //echo "<div class='alert alert-success'>Successfully submitted..</div>";
         echo "<script>window.location='restaurantRequest.php?status=ok';</script>";
       }
}
...
<div class="page-container"><!-- add class "sidebar-collapsed" to close sidebar by default, "chat-visible" to make chat appear always -->	
	...
	<div class="row">
        ...
        <div class=""><h2 class="toast-title">Restaurant Request</h2></div>
        ...
        <br>
         <table class="display nowrap table table-hover table-striped table-bordered" id="table-1">
            ...
            <tbody>
                <?php 
                $url = $baseurl."/showRestaurantRequests";
                $params = "";
                ...
                foreach($json_data['msg'] as $str => $data) 
				{
                    //var_dump($data);
					 ?>
						<td><?php echo $data['RestaurantRequest']['id']; ?></td>
						<td><?php echo $data['RestaurantRequest']['restaurant_name']; ?></td>
						<td><?php echo $data['RestaurantRequest']['contact_name']; ?></td>
						<td><?php echo $data['RestaurantRequest']['email']; ?></td>
						<td><?php echo $data['RestaurantRequest']['phone']; ?></td>
						<td><?php echo $data['RestaurantRequest']['address']; ?></td>
						<td><?php echo $data['RestaurantRequest']['description']; ?></td>
            <td>
                <a href='?removeRestaurantRequest=ok&id=<?php echo $data['RestaurantRequest']['id'] ?>'  onClick="return confirm('Are you sure?')" class='btn btn-default btn-sm'>
